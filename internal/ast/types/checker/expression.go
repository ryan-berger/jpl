...
func checkIf(ifExpr *ast.IfExpression, table *symbol.Table) (types.Type, error) {
	...
	if !condType.Equal(types.Boolean) {
		return nil, NewError(ifExpr.Condition, "type error: expected boolean, received %s", condType)
	}
	...
	if !consType.Equal(otherType) {
		return nil, NewError(ifExpr, "type error: branches return different types: %s, %s", consType, otherType)
	}
